...
def main():
    ...
    for parsed_file in parsed_files:
        new_relative_path = parsed_file.file_path[len(source_dir) + 1:-(len(config.header_extension))] + f"meta."
    ...
    if args.watch:
        def generate_file(file_path: str):
            ...
            if re_parsed is not None:
                new_relative_path = re_parsed.file_path[len(source_dir) + 1:-(len(config.header_extension))] + f"meta."
                out_file_path = os.path.join(output_dir,new_relative_path)
                dir_name = os.path.dirname(out_file_path)
                if not os.path.exists(dir_name):
                    os.makedirs(dir_name)
