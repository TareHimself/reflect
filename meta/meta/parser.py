...
from meta.constants import PROPERTY_REGEX, FUNCTION_REGEX, ARGUMENT_REGEX
...
class FileParser:
    ...
    def pre_process(self):
        ...
        while line is not None:
            if line.startswith("namespace") and "{" in line:
                temp = line[len("namespace ") : line.index("{") + 1].split(" ")

                namespaces.append((temp[0], self.line_number, -1))
                self.depth += 1
    ...
    def parse_struct(self, macro_line: str) -> Union[None, ParsedStruct]:
        ...
        split_result = line[line.index(token_found) + len(token_found) :].split(" ")

        if len(split_result) == 0:
            return None

        result = ParsedStruct(self.parse_tags(macro_line))
    ...
    def parse(self) -> Union[None, ParsedFile]:
        ...
        while line is not None:
            if self.config.struct_tag in line:
