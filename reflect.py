...
REFLECT_ARGUMENT_REGEX = (
    "(?:const[\s]+?)?([a-zA-Z:0-9_*\s<>]+)(?:[&\s]+)([a-zA-Z0-9_]+)"
)
...
def write_property(file: TextIOWrapper, type_name: str, field: ParsedProperty) -> str:
    ...
    write_line(
        file,
        f"#define {macro} REFLECT_WRAP_PROPERTY({type_name},{field.name},{field.type})",
    )
...
def write_fields(
    file: TextIOWrapper, type_name: str, fields: list[ParsedField]
) -> list[str]:
    macros: list[str] = []
...
def search_for_headers(dir: str, header_ext: str) -> list[str]:
    ...
    for entry in os.listdir(dir):
        ...
        elif entry.endswith(f".{header_ext}") and not entry.endswith(
            f"reflect.{header_ext}"
        ):
...
def main():
    ...
    for parsed_file in parsed_files:
        new_relative_path = (
            parsed_file.file_path[len(source_dir) + 1 : -(len(filter_ext))]
            + f"reflect.{filter_ext}"
        )
